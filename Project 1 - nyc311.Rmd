...
agency$pay = sum(samp_city_pay$base_salary) + sum(samp_city_pay$total_ot_paid)
...
```{r}
#updating the DF to have 30 values 
...
for (i in 1:49) {
  ...
  temp_agency$pay = sum(samp_city_pay$base_salary) + sum(samp_city_pay$total_ot_paid)
...
model = lm(y1 ~ x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 + x10 + x11 + x12 + x13 + x14 + x15 + x16)
summary(model)
```
