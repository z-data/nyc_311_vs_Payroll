...
```{r}
#Function for changing names of the DF 
...
Engineered the new feature of the data 
```{r}
...
total
```
Making a more robust data frame 
```{r}
...
#it was easier than changing all the names again
...
rownames(per_comp) = c("per complaint", "total complaints", "average salary" , "factor", "max salary", "employee count")
...
per_comp = as.data.frame(t(per_comp))
per_comp = as.data.frame(scale(per_comp))
cor(per_comp, use = "complete.obs")
```

```{r}
#this summary is about employee vs 
summary(lm(per_comp$`employee count` ~ ., data = per_comp))
```
...
```{r}
model = summary(lm(per_comp$`employee count` ~ ., data = per_comp))
model_coef = append(model_coef , coef(model)[,1])
model_coef
```
```{r}
#i ran this code multiple times by hand to get these values
#i could have made a loop, but this depts not lining up had me sketched out
#so this is 5 random samplings of the data, I could have done more, but it is a good rep of the data 


values = c()
for (i in 1:6) {
  v = 3 + 6*(i-1)
  values = append(values, model_coef[v] )
}


mean(values)
plot(values,  ylab = "Coeff values",  main = "values of coefficient of total complaints")
...
```{r}
summary(lm(per_comp$`total complaints` ~ ., data = per_comp))
```

```{r}
summary(lm(per_comp$factor ~ ., data = per_comp))
```
```{r}
summary(lm(per_comp$`per complaint` ~ ., data = per_comp))
```
```{r}
summary(lm(per_comp$`average salary` ~ ., data = per_comp))
```
